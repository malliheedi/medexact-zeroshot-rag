...
def call_api(model, segment, base_url, api_key, timeout):
    ...
    # By default OFF (no_think) for Qwen3/DeepSeek — use --think to enable
    _thinking_models = ("qwen3", "qwen/qwen3", "deepseek")
    _is_thinking_model = any(t in model.lower() for t in _thinking_models)
    _think_enabled = getattr(args, "think", False)

    if _is_thinking_model and not _think_enabled:
...
def main():
    ...
    p.add_argument("--think", action="store_true", default=False,
        help="Enable thinking mode for Qwen3/DeepSeek (default: OFF = no_think)")
    args = p.parse_args()
