...
MODEL_PROFILES = {
    ...
    "llama3.3:70b":      {"chunk_size": 8000, "overlap": 400, "note": "A6000 max upgraded"},
    "phi4:14b":          {"chunk_size": 4000, "overlap": 300, "note": "Fast clinical logic"},
    
    # --- NEW A6000 MODELS ---
    "glm4:9b":           {"chunk_size": 4000, "overlap": 300, "note": "Strong GLM architecture"},
    "command-r:35b":     {"chunk_size": 6000, "overlap": 300, "note": "Top-tier RAG span extraction"},
    "deepseek-r1:32b":   {"chunk_size": 5000, "overlap": 300, "note": "Chain-of-thought logic"}
}
...
def call_ollama(model: str, prompt: str, base_url: str, timeout: int, num_ctx: int):
    ...
    # This ensures the model treats instructions as system-level, not user-level
...
def load_index(index_path: Path):
    ...
    if not (idx_f := index_path / INDEX_FILE).exists() or not (chk_f := index_path / CHUNKS_FILE).exists(): 
        console.print(f"  [red]Index not found[/red]")
        sys.exit(1)
    index = faiss.index_cpu_to_gpu(faiss.StandardGpuResources(), 0, faiss.read_index(str(idx_f)))
...
def process_document(txt_path, output_dir, args, index, chunks, embedder, doc_index, total_docs, progress, task_id):
    ...
    for ci, (chunk_start, chunk_text) in enumerate(chunks_list, 1):
        ...
        try: raw, _ = call_ollama(args.model, prompt, args.base_url, args.timeout, args.num_ctx)
        except Exception: 
            progress.update(task_id, advance=1)
            continue
...
def main():
    ...
    run_parser.add_argument("--timeout", type=int, default=400)
    ...
    all_scores, txt_files = [], sorted(data_dir.glob("*.txt"))

    # NEW: Filter files if an ID list is provided
    if hasattr(args, 'ids') and args.ids and Path(args.ids).exists():
        valid_ids = {line.strip().replace('.txt', '') for line in Path(args.ids).read_text().splitlines() if line.strip()}
        txt_files = [f for f in txt_files if f.stem in valid_ids]
    ...
    if (agg := aggregate(all_scores)): 
        print_summary(agg)
        summary_path = output_dir / f"{args.team_id}-eval-summary.json"
        summary_path.write_text(json.dumps({"aggregate": agg}, indent=4), encoding="utf-8")
