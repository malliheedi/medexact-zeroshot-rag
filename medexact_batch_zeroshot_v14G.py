"""
MedExACT Batch Zero-Shot Pipeline v14 (A6000 Final Run - API FIXED)
"""
...
MODEL_PROFILES = {
    ...
    "moonshotai/kimi-k2-instruct":                   {"chunk_size": 1500, "overlap": 150, "note": "Kimi API"},
    "meta/llama-3.3-70b-instruct":                   {"chunk_size": 1500, "overlap": 150, "note": "Llama API"},
    "mistralai/mistral-large-3-675b-instruct-2512":  {"chunk_size": 1500, "overlap": 150, "note": "Mistral API"},
    "deepseek-ai/deepseek-v3.1":                     {"chunk_size": 1500, "overlap": 150, "note": "DeepSeek API"},
    "qwen/qwen3.5-122b-a10b":                        {"chunk_size": 1500, "overlap": 150, "note": "Qwen API"},
    "gemini-2.5-flash":                              {"chunk_size": 1500, "overlap": 150, "note": "Gemini API"},
    "ibm/granite-3.3-8b-instruct":                   {"chunk_size": 800,  "overlap": 100, "note": "Granite API limit - STRICT"}
}
...
SECTION_HEADERS = [
    ...
    "hpi", "ros", "pmh", "pmhx", "psh", "pshx", "cc", "fh", "sh", "pe", "bhc",
...
4. NEGATIVE CONSTRAINTS (WHAT NOT TO EXTRACT): Do NOT extract standard nursing tasks or generic hospital routines. Do NOT extract historical narratives that are not active problems.
...
def call_api(model, segment, base_url, api_key, timeout, think_enabled):
    ...
    _thinking_models = ("qwen3", "qwen/qwen3", "deepseek")
    _is_thinking_model = any(t in model.lower() for t in _thinking_models)

    if _is_thinking_model and not think_enabled:
        user_content       = "/no_think\n" + segment
        extra_payload_args = {"enable_thinking": False}
    else:
    ...
    payload = {
        ...
        "temperature": 0.0,
        "max_tokens":  1024, # STRICT LIMIT: Prevents Solar 400 errors and 504 Timeouts
        **extra_payload_args,
    }
    ...
    for attempt in range(1, 4):
        try:
            ...
            # Explicitly print NIM API rejections to the terminal so we can see them
            if r.status_code != 200:
                console.print(f"\n[bold red]API REJECTED REQUEST (Code {r.status_code}): {r.text}[/bold red]")
            ...
            if attempt == 3: raise Exception(f"API Failed after 3 attempts: {e}")
...
def process_document(txt_path, output_dir, args, doc_index, total_docs, progress, task_id):
    ...
    for ci, (chunk_start, chunk_text) in enumerate(chunks, 1):
        ...
        try:
            ...
            raw, _ = call_api(args.model, chunk_with_context, "https://integrate.api.nvidia.com/v1", 
                              args.api_key, args.timeout, getattr(args, 'think', False))
            if getattr(args, 'api_delay', 0) > 0:
                time.sleep(args.api_delay)
        except Exception as e:
            err_msg = f"chunk {ci} FAILED: {e}"
            logging.error(err_msg)
            console.print(f"\n  [bold red]CRITICAL ABORT on {doc_id} - {err_msg}[/bold red]")
            raise Exception(f"Document processing aborted due to API failure: {e}")
